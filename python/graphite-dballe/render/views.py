...
def renderView2(request):
    ...
    start_time = min([series.start for series in data])
    end_time = max([series.end for series in data])

    response = {"datasets": [], "start_time": start_time, "end_time": end_time, "max_lenght": max_length}
    labels = []
    for series in data:
        data_dict = []
        for point in series.datapoints():
            data_dict.append({"x": datetime.fromtimestamp(point[1]).strftime("%d/%m/%Y, %H:%M:%S"), "y": point[0]})
            if point is not None:
                labels.append(datetime.fromtimestamp(point[1]).strftime("%d/%m/%Y, %H:%M:%S"))
        response["datasets"].append(
            {"data": data_dict, "fill": False,
...
def renderViewJson(requestOptions, data):
    ...
    output = json.dumps(series_data, indent=(2 if requestOptions.get('pretty') else None)).replace('None,',
                                                                                                   'null,').replace(
        'NaN,', 'null,').replace('Infinity,', '1e9999,')
